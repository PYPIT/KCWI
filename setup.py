...
#
setup_keywords['name'] = 'pyp_kcwi'
setup_keywords['description'] = 'PYPIT KCWI Spectroscopic Reduction'
setup_keywords['author'] = 'PYPIT Collaboration'
setup_keywords['author_email'] = 'pypit@ucolick.org'
setup_keywords['license'] = 'BSD'
setup_keywords['url'] = 'https://github.com/PYPIT/KCWI'
#
...
pyx_files = glob.glob('pyp_kcwi/*.pyx')
...
for pyx_file in pyx_files:
    ...
    ext = Extension('pyp_kcwi.'+pyx_split2[1], [pyx_file],
...
data_generator = os.walk('pyp_kcwi/data')
...
settings = glob.glob('pyp_kcwi/settings/settings.*')
settings = ['/'.join(path.split('/')[1:]) for path in settings]
data_files.extend(settings)
setup_keywords['package_data'] = {'pyp_kcwi': data_files,
                                  '': ['*.rst', '*.txt']}
